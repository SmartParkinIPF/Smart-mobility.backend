...
router.get("/public/:id/geometry", controller.getPublicGeometry);

// Resumen + listado de slots por establecimiento (agregado para mobile)
router.get("/:id/slots", JwtValidator.validateJwt, async (req, res, next) => {
  try {
    ...
    const occSet = new Set(reservas.map((r: any) => r.slot_id).filter(Boolean));
